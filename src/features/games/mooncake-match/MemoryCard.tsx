  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={cn(
            'flip-card absolute inset-0 flex items-center justify-center rounded-2xl border border-gold-400/25 bg-gradient-to-br from-night-700 to-night-900 shadow-glass transition',
            !shown && !disabled && 'hover:border-gold-400/60 hover:shadow-gold-sm',
            reduced && shown && 'opacity-0',
          )}
        >
          <span className="font-display text-3xl text-gold-400/70">月</span>
        </div>
        {/* front */}
        <div
          className={cn(
            'flip-card absolute inset-0 flex flex-col items-center justify-center gap-1 rounded-2xl border p-2 shadow-glass',
        ...
        >
          <div className="relative size-3/5">
            <MooncakeIcon color={flavor.color} className="size-full drop-shadow-[0_0_10px_rgba(255,209,102,0.35)]" />
            <Icon className="absolute inset-0 m-auto size-1/3 text-night-950/80" strokeWidth={2.5} />
          </div>
          <span className="text-[10px] font-semibold text-cream/80 sm:text-xs">{t(flavor.labelKey)}</span>
        </div>
      </motion.div>
    </button>
  )
